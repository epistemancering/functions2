...
let findGrapeExpression = function(array) {
    for (let index in array) {
        if (array[index].color === `purple`) {
            return console.log(`The fruit with index ${index} is a grape`)
        }
    }
}

// RE-WRITE THE ABOVE FUNCTION IN 'ARROW FUNCTION' SYNTAX HERE.
let findGrapeArrow = (array) => {
    for (let index in array) {
        if (array[index].color === `purple`) {
            return console.log(`The fruit with index ${index} is a grape`)
        }
...
// CODE HERE
let print = (parameter1, parameter2) => console.log(`The first parameter is ${parameter1}. The second parameter is ${parameter2}.`)
...
greeting(`Austin`, `Henrie`, function (fullName) {
...
function pricesPlusTax(array, callback) {
    for (let index in array) {
        totalCost[index] = 1.2 * array[index]
    }
    callback(totalCost)
}
...
// CODE HERE
pricesPlusTax(prices, function (array) {
    console.log(`The new array plus tax = ${array}`)
})
...
// CODE HERE
function multiplyingFactory(first) {
